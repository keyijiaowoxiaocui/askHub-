  ...
  <div class="main2">
    <div class="main-top">
      <div class="main-top-top">
        <span>1. 上传商品图片</span>
        <span>重新上传 最多5张图片</span>
        <div class="sell-img">
          <div>
            <div class="image-upload">
              <div style="display: flex;" v-if="uploadImg.uploadedImageUrl" class="image-upload-left">
                <div v-for="imageUrl in uploadImg.uploadedImageUrl" :key="imageUrl.index" @click="handleDelete(imageUrl)">
                  <span class="xian">
            ...
            <!-- <div>
              ...
              <div v-if="uploadImg.uploadedImageUrl" style="display: flex">
                <img v-for="imageUrl in uploadImg.uploadedImageUrl" :key="imageUrl.index" :src="imageUrl"
                  alt="Uploaded Image" class="sub_img" />
              </div>
            </div> -->
          </div>
        </div>
        <div style="font-size: 18px">
          ...
          <div>
            <!-- ￥： -->
            <!-- <el-input style="background-color: #d9d7da" v-model="price" type="number" class="create-input"
            ...
            <div style="margin-top:20px ;">￥： <input type="number"
                ...
                class="create-input" oninput="value=value.replace(/[^\d.]/g,'')" placeholder="请输入内容">
    ...
    <div class="main-bottom">
      ...
      <div class="main-bottom-b">
        <div style="overflow: auto; margin-right: 50px">
          <ul class="group-wrap">
            <li class="group-item">
        ...
        <div>
          ...
          <textarea style="width: 400px; font-size: 18px; background-color: #ffffff" v-model="shopIntroduce" name="beizhu"
            clos="60" rows="15" maxlength="180"></textarea>
  ...
  <div class="category-path">
    <div class="path-info-wrap">
      <div class="path-list">
        ...
        <button id="leiMuTi" @click="submit">提交</button>
      </div>
    </div>
  </div>
</template>
...
import { getupdeleteCommodity } from "@/api/shop/shop";
import { getAllCommodityByBelongUserId } from "../../api/my-api";
...
export default {
  ...
  setup() {
    ...
    const getUser = async () => {
      ...
      aa.belong_user_id = data.data.user_id;

      const res2 = await getAllCommodityByBelongUserId(aa);
      console.log("个人商品列表", res2);
      shop = res2.data.data;
      muBiaoShop.value = shop.filter(
        (item) => item.commodity_id == Route.query.shopid
      )[0];
      let linShi = shop.filter(
        (item) => item.commodity_id == Route.query.shopid
      )[0];
      // 初始赋值
      console.log(linShi);
      uploadImg.uploadedImageUrl = linShi.commodity_images;
      pictureShu.value = pictureShu.value - uploadImg.uploadedImageUrl.length;

      price.value = linShi.price;
      shopName.value = linShi.name;
      LeiMu.value = linShi.classify;
      shopIntroduce.value = linShi.introduction;
    };
    ...
    const handleFileUpload = (event) => {
      ...
      if (uploadImg.uploadedImageUrl.length === 5) {
        alert("图片已达上限");
        return;
      }
    ...
    const uploadimg = () => {
      if (uploadImg.uploadedImage) {
        ...
        for (var i = uploadImg.uploadedImageUrl.length - 1; i < uploadImg.uploadedImageUrl.length + 1; i++) {
    ...
    const submit = async () => {
      ...
      if (
        shop_data.belong_user_id === "" ||
        shop_data.commodity_images.length === 0 ||
        shop_data.name === "" ||
        shop_data.price < 0 ||
        shop_data.price === "" ||
        shop_data.introduction === "" ||
        shop_data.classify === ""
      ) {
        alert("信息不全");
        return;
      ...
      const res = await getupdeleteCommodity(shop_data);
      if (res.data.message == "success") {
        ...
        alert("更新成功");
        // location.reload();
        router.push("/shop/" + shop_data.commodity_id)
    ...
    return {
      ...
      handleFileUpload,
      uploadImg,
      Route, handleDelete, changeStyle
    };
  },
};
</script>
