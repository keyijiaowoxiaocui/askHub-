<template>
  <div class="app-top-nav">
    <ul class="left">
      ...
      <li v-for="(item, index) in navItems" :key="index" @click="goToRoute(item.path)">
        {{ item.name }}
      </li>
    </ul>
    <div class="search_box" v-if="getrouter.go_router !== '/my/lately'">
      <!--input搜索框-->
      <input v-if="getrouter.go_router != '/msg'" type="text" class="search_text" placeholder="不如探索一下AskHub?"
        v-model="inpval" @input="handleInputChange" @keyup.enter="enterSearch" style="color: black;" />
      <!-- 商品页面搜索框 -->
      <router-link to="/homeSearchResult">
        <a v-if="getrouter.go_router == '/'" class="search_btn" @click="homeSearch(inpval)"><span
            class="iconfont icon-sousuo"></span>
        </a>
      </router-link>
      <!-- <router-link to="/homeSearchResult"> -->
      <a v-if="getrouter.go_router == '/shop'" class="search_btn" @click="shopSearch(inpval)"><span
          class="iconfont icon-sousuo"></span>
      </a>
      <!-- </router-link> -->
      <router-link to="/homeSearchResult">
        <a v-if="getrouter.go_router.includes('article')" class="search_btn" @click="articleSearch(inpval)"><span
            class="iconfont icon-sousuo"></span></a>
      </router-link>
      <a v-if="getrouter.go_router == '/homeSearchResult'" class="search_btn" @click="homeSearch(inpval)"><span
          class="iconfont icon-sousuo"></span>
      </a>
    </div>
    <ul id="right">
  ...
  <el-tooltip class="box-item" effect="dark" content="工单中心，解疑答惑" placement="left-start">
...
export default {
  setup() {
    ...
    const adc = () => {
      return getrouter.go_router.includes('msg') ? false : true
    }
    ...
    const enterSearch = function () {
      document.onkeydown = e => {
        ...
        if (e.keyCode === 13) {
          //回车后执行搜索方法
          if (e.target.baseURI.includes('article')) {
            console.log(123132);
            articleSearch(inpval)
            router2.push('/homeSearchResult')
          }
          if (e.target.baseURI.includes('shop')) {
            shopSearch(inpval)
          }
          try {
            if (e.target.baseURI == "http://localhost:8080/#/") {
              homeSearch(inpval)
              router2.push('/homeSearchResult')
            }
          } catch (error) {
