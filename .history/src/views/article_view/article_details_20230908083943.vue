...
export default {
	...
	setup() {
		import { ref, onMounted, onBeforeUnmount, onScroll } from 'vue';

		const activeName = ref('tab0');
		const tabPosition = 'right';
		const scroll = ref(0);
		const list = ref([
			{ name: '第一条', backgroundcolor: '#90B2A3' },
			{ name: '第二条', backgroundcolor: '#A593B2' },
			{ name: '第三条', backgroundcolor: '#A7B293' },
			{ name: '第四条', backgroundcolor: '#0F2798' },
			{ name: '第五条', backgroundcolor: '#0A464D' },
		]);
		const navList = ref([1, 2, 3, 4, 5]);
		...
		const jump = (index) => {
			const jumpElements = document.getElementsByClassName('section');
			const total = jumpElements[index].offsetTop;
		...
		const loadScroll = () => {
			...
			for (let i = sections.length - 1; i >= 0; i--) {
				if (scroll.value >= sections[i].offsetTop - 100) {
