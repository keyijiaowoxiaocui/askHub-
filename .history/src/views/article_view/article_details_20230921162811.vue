  ...
  setup () {
    ...
    onMounted(() => {
      ...
      getArticleDetails(articleID.value, usId.value).then((res) => {
        ...
        console.log('fasdjkfhadjklshjkahdsfjklasdhjfk', tocommentData.belong_user_id, tocommentData.styleId)
        // console.log(
    ...
    const imgsize = function (img) {
      // 使用正则表达式匹配<img>标签
      const regex = /<img[^>]*\b(?:width|height)\s*=\s*["']?(\d{3,}|[1-9][0-4]\d{2,})["']?\s*[^>]*>/g
        ...
        for (const imgTag of matchedImages) {
          ...
          if (widthMatch && heightMatch) {
            ...
            if (width > 750) {
              ...
              // 替换原始的<img>标签
              articleData.value.content = articleData.value.content.replace(imgTag, updatedImgTag)
            }
          }
        }
      }
