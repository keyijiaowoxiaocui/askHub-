  ...
  <div class="left_note w" v-for="(item, index) in items" :key="index">
      ...
      <div class="avatar">
        <router-link
          :to="{
            ...
            query: { OtherUserId: item.belongUserID },
          }"
        >
    ...
    <div class="content" v-if="index % 2 !== 0">
      ...
      <span>
        <router-link
          :to="// {
          // 	path: `/OtherUsers/OtherLately?OtherUserId=${item.belongUserID}`
          // 	// query: { OtherUserId: item.belongUserID }
          // }
          {
            path: '/OtherUsers/OtherLately',
        ...
        >
        <!-- <span @click="click_to_other(index)"> {{ item.user.nick_name }}
				</span> -->
        Write Down
...
export default {
  setup () {
    ...
    // const click_to_other = (index) => {
    // 	router.push({ path: '/OtherUsers', query: { OtherUserId: items.value[index].user.user_id } });
