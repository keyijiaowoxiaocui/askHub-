			...
			<div class="entry">
				...
				<div class="content_main">
					...
					<div class="container_bottom">
						...
						<div class="img_box" v-show="item.thumbnail">
							<img :src="item.user.avatar_path" alt="头像" />头像
						</div>
					</div>
	...
	setup() {
		...
		watch(
			...
			(newValue, oldValue) => {
				...
				Column.getArticleData(categoryId, 0, 3);
			},
			{ immediate: true }
		);
		onMounted(async () => {
			// await Column.getArticleData(categoryId, 0, 3); // 加载初始的三条数据
			loadedDataCount.value = 3; // 初始化已加载数据数量
			console.log(2222222);
